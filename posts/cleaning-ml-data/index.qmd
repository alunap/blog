...
First load it, dealing with the column names in the process. I used DrWatson to set up my Julia projects.
```{{julia}}
using DrWatson
...
This worked well for scikit-learn models. I also wanted to build a Distilbert model using hugging-face transformers. Scikit-learn has a nice feature to split test-train and then uses cross-validation, but hugging face apparently does not. I split the training set into a train/validation set.
```{{julia}}
